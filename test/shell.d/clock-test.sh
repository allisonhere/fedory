...
// ---- widget wiring
assert(/moduleName: "fedory\.clock"/.test(panelSource), 'calendar panel declares its module name')
assert(/ipcTarget: "fedory\.clock"/.test(panelSource), 'calendar panel registers its IPC target')
...
assert(/fedory-menu-timezone/.test(widgetSource), 'clock keeps the timezone picker on middle click')
...
shell_json=$(cd "$ROOT_DIR" && jq -r '[.bar.layout.center[].id] | join(",")' config/fedory/shell.json)
[[ $shell_json == *"fedory.clock"* ]] || fail "default bar layout includes the clock widget" "center: $shell_json"
[[ $shell_json != *"fedory.calendar"* ]] || fail "clock hosts the calendar instead of a second bar pill" "center: $shell_json"
pass "default bar layout includes the clock widget"

grep -q 'o.bind("SUPER + CTRL + ALT + D", "Calendar", "fedory-shell shell toggle fedory.clock")' \
