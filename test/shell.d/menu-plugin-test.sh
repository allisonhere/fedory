...
# The picker reads the plugin list from fedory-plugin-list and hands what it
...
cat >"$STUB_DIR/fedory-plugin-list" <<'STUB'
#!/bin/bash
cat "$FAKE_PLUGINS"
STUB

for command in fedory-plugin-enable fedory-plugin-disable; do
...
cat >"$STUB_DIR/fedory-menu-select" <<'STUB'
#!/bin/bash
cat >"$FAKE_ROWS"
printf '%s\n' "$FAKE_PICK"
STUB

cat >"$STUB_DIR/fedory-notification-send" <<'STUB'
#!/bin/bash
printf 'notification: %s\n' "$*" >>"$FAKE_CALLS"
STUB

cat >"$STUB_DIR/fedory-launch-floating-terminal-with-presentation" <<'STUB'
#!/bin/bash
printf 'terminal: %s\n' "$*" >>"$FAKE_CALLS"
STUB
...
pick() {
  ...
  HOME="$TMPDIR/home" \
    ...
    FAKE_PICK="$choice" \
    "$ROOT_DIR/bin/fedory-menu-plugin" "$verb" >/dev/null 2>&1
...
[
  {"id": "fedory.clock", "name": "Clock", "kinds": ["bar-widget"], "enabled": false, "active": false, "canDisable": true, "firstParty": true},
...
cat >"$TMPDIR/plugins.json" <<'JSON'
[
  {"id": "fedory.clock", "name": "Clock", "kinds": ["bar-widget"], "enabled": true, "active": false, "canDisable": true, "firstParty": true},
  {"id": "tester.clock", "name": "Clock", "kinds": ["bar-widget"], "enabled": false, "active": false, "canDisable": true, "firstParty": false}
]
JSON
...
pick remove "Clock"
[[ $CALLS == *"fedory-plugin-remove tester.clock"* ]] \
...
[[ $CALLS == *"fedory-plugin-enable acme.weather"* ]] \
...
[[ $CALLS == *"terminal: fedory-plugin-clone --edit fedory.clock"* ]] ||
...
[
  {"id": "fedory.clock", "name": "Clock", "kinds": ["bar-widget"], "enabled": true, "active": false, "canDisable": true, "firstParty": true},
  {"id": "tester.clock", "name": "My Clock", "kinds": ["bar-widget"], "enabled": false, "active": false, "canDisable": true, "firstParty": false, "clonedFrom": "fedory.clock"}
]
JSON
...
pick enable "Fancy"
[[ $CALLS == *"fedory-plugin-enable acme.fancy"* && $CALLS != *"--section"* ]] \
...
cat >"$TMPDIR/plugins.json" <<'JSON'
[
  {"id": "fedory.bar", "name": "Bar", "kinds": ["bar"], "enabled": false, "active": false, "canDisable": false, "firstParty": true},
...
[[ $CALLS == *"fedory-plugin-enable fedory.bar"* ]] \
...
cat >"$TMPDIR/plugins.json" <<'JSON'
[
  {"id": "fedory.bar", "name": "Bar", "kinds": ["bar"], "enabled": true, "active": true, "canDisable": false, "firstParty": true}
]
JSON
