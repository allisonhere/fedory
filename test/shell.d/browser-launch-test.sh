...
printf '%s\n' "\$*" > "$launch_log"
EOF
chmod +x "$fake_bin/xdg-settings" "$fake_bin/setsid" "$fake_bin/systemd-run"
...
assert_eq "uwsm app -- flatpak run org.chromium.Chromium --app=https://example.com" \
...
if [[ $browser_launch == *"uwsm app -- flatpak run org.chromium.Chromium --incognito https://example.com" ]]; then
