...
if perl -0ne 'exit(/drag\s*\.\s*target\s*:\s*[^;]*\bslot\b/s ? 0 : 1)' "$ROOT_DIR/shell/plugins/bar/Bar.qml"; then
...
if rg -q 'barMoveSettling|barMoveSettleTimer' "$ROOT_DIR/shell/plugins/bar/Bar.qml"; then
...
const drawn = { moduleName: 'fedory.clock', visible: true, width: 28, height: 81 }
const placeholder = { moduleName: 'fedory.clock', visible: false, width: 0, height: 0 }
...
assertDeepEqual(bar.entrySettings({ id: 'fedory.clock', format: 'HH:mm' }), { format: 'HH:mm' }, 'bar extracts entry settings')
assertEqual(bar.entryId({ id: 'fedory.clock' }), 'fedory.clock', 'bar extracts object entry ids')
assertEqual(bar.entryId('fedory.clock'), 'fedory.clock', 'bar extracts string entry ids')

const entries = [{ id: 'a' }, { id: 'fedory.tray' }, { id: 'b' }]
assertDeepEqual(bar.pinTrayToInner(entries, 'left').map(bar.entryId), ['a', 'b', 'fedory.tray'], 'bar pins tray to left inner edge')
assertDeepEqual(bar.pinTrayToInner(entries, 'right').map(bar.entryId), ['fedory.tray', 'a', 'b'], 'bar pins tray to right inner edge')
...
const settingsLayout = { left: [{ id: 'fedory.power' }], center: [{ id: 'fedory.clock', format: 'HH:mm' }], right: [] }
assertDeepEqual(
  bar.inlineSettingsDelta(settingsLayout, { left: [{ id: 'fedory.power', showPercentage: true }], center: [{ id: 'fedory.clock', format: 'HH:mm' }], right: [] }),
  [{ region: 'left', index: 0, entry: { id: 'fedory.power', showPercentage: true } }],
...
assertEqual(
  bar.inlineSettingsDelta(settingsLayout, { left: [{ id: 'fedory.clock', format: 'HH:mm' }], center: [{ id: 'fedory.power' }], right: [] }),
  null,
  'bar treats reordered entries as structural'
)
assertEqual(
  bar.inlineSettingsDelta(settingsLayout, { left: [{ id: 'fedory.power' }, { id: 'fedory.battery' }], center: settingsLayout.center, right: [] }),
  null,
...
assertDeepEqual(bar.entriesBefore(entries, 'b').map(bar.entryId), ['a', 'fedory.tray'], 'bar returns entries before target')
assertDeepEqual(bar.entriesAfter(entries, 'a').map(bar.entryId), ['fedory.tray', 'b'], 'bar returns entries after target')
...
assertEqual(
  bar.customModulePath({ id: 'local.weather' }, '/home/dhh', '/home/dhh/.config/fedory'),
  '/home/dhh/.config/fedory/bar/modules/local.weather.qml',
