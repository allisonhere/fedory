...
cat >"$mock_bin/fedory-hyprland-monitor-focused-apple" <<'SH'
#!/bin/bash
exit 1
SH

cat >"$mock_bin/fedory-hyprland-monitor-focused" <<'SH'
#!/bin/bash
printf '%s\n' "${FOCUSED_MONITOR:-eDP-1}"
SH

cat >"$mock_bin/fedory-hw-display" <<'SH'
#!/bin/bash
...
run_brightness() {
  CALL_LOG="$call_log" XDG_RUNTIME_DIR="$runtime_dir" PATH="$mock_bin:$ROOT_DIR/bin:$PATH" \
    "$ROOT_DIR/bin/fedory-brightness-display" "$@"
}
...
rm -f "$runtime_dir/fedory-brightness-display-ddc/DP-1.bus"
detect_count=$(grep -c ' detect --brief' "$call_log")
...
printf '7 80 0\n' >"$runtime_dir/fedory-brightness-display-ddc/DP-1.bus"
get_count=$(grep -c ' getvcp 10 ' "$call_log")
...
PATH="$mock_bin:$PATH" "$ROOT_DIR/bin/fedory-hyprland-monitor-focused-apple" DP-2 || \
  fail "named Apple display is detected independently of focus"
if PATH="$mock_bin:$PATH" "$ROOT_DIR/bin/fedory-hyprland-monitor-focused-apple"; then
