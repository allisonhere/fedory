...
exit 0
EOF
chmod +x "$tmp/bin/nmcli" "$tmp/bin/qrencode" "$tmp/bin/ip"
...
run_success_case() {
  ...
  output=$(PATH="$tmp/bin:$PATH" "$ROOT_DIR/bin/fedory-network-qr" "$@")
  expected=$'100\n010\n001'
...
if PATH="$tmp/bin:$PATH" "$ROOT_DIR/bin/fedory-network-qr" wlan0 >"$tmp/enterprise-output" 2>"$tmp/enterprise-error"; then
