...
# Fresh Fedory uses NetworkManager. Some installers' "copy live-image
# network config" modes enable systemd-networkd and drop DHCP .network files
# that compete with NetworkManager, so retire that state whenever hardware
# setup runs (harmless no-op if nothing matches, e.g. a plain Fedora
# Workstation install where NetworkManager already owns networking).
for unit in \
...
backup_dir="/etc/systemd/network/fedory-networkd-retired-$(date +%Y%m%d%H%M%S)"
