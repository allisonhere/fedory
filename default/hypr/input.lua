-- Based on basecamp/omarchy default/hypr/input.lua with Fedory command and path names.
...
-- Layouts that can't type Latin letters. Fedora reads this directly from
-- vconsole.conf; there is no mkinitcpio keyboard hook to keep in sync.
...
-- layout that's currently active, so Fedory's Latin-keysym bindings (SUPER + W
