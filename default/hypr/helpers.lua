-- Based on basecamp/omarchy default/hypr/helpers.lua with Fedory command and path names.
...
local function command_from(value, description)
  ...
  if value.fedory then
    return "fedory-launch-" .. value.fedory
  ...
  elseif value.tui then
    if value.focus then
      return "fedory-launch-or-focus-tui " .. shell_quote(value.tui)
    else
      return "fedory-launch-tui " .. shell_quote(value.tui)
    end
  end
...
function o.preinstalled_bindings_enabled()
  if _G.fedory_preinstalled_bindings ~= nil then
    return _G.fedory_preinstalled_bindings == true
  end

  return not file_exists((os.getenv("HOME") or "") .. "/.local/state/fedory/preinstalls-removed")
end
...
function o.launch_webapp(url)
  return "fedory-launch-webapp " .. shell_quote(url)
end

function o.launch_webapp_sole(name, url)
  return "fedory-launch-or-focus-webapp " .. shell_quote(name) .. " " .. shell_quote(url)
end

function o.launch_sole(match, command)
  return "fedory-launch-or-focus " .. shell_quote(match) .. " " .. shell_quote(o.launch(command))
end

function o.bind_toggle(keys, description, toggle, options)
  o.bind(keys, description, "fedory-toggle-" .. toggle, options)
end

function o.notify(message)
  return "fedory-notification-send -u low " .. shell_quote(message)
end
