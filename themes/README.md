...
Fedory instead ships five pieces of original artwork under
`default/wallpapers/`. Each theme links its `backgrounds/fedory.png` to the
family that best matches its palette. `fedory-theme-set` dereferences that
link while staging the active theme, keeping the runtime state self-contained
without storing 22 duplicate PNG files in Git.
