...
| `bar`        | Full bar option that can replace `fedory.bar` |
...
Only one full bar option is active at a time. The built-in `fedory.bar` is
...
clones it straight into `~/.config/fedory/plugins/<id>/`; updating is a
fast-forward pull:

```bash
fedory plugin add https://github.com/acme/fedory-weather.git
fedory plugin update                # fetches, shows a diff, fast-forwards
...
Plugins run as **unsandboxed code** inside `fedory-shell`. Adding warns you
before cloning, plugins land disabled so you can review the code before
`fedory plugin enable`, and updates show a diff before touching anything.
...
`~/.config/fedory/plugins/<id>/`, run `fedory-shell shell rescanPlugins`, then
`fedory plugin enable <id>`. A bar widget starts in its declared default
section and can be moved with `fedory bar move`; enabling a full bar
replaces the one in use.
The lower-level IPC methods remain available through `fedory-shell shell ...`.
...
1. The active bar option is `bar.id`. Omit it or set it to `fedory.bar` for
...
3. Settings are inline on the entry. No `config:` sub-object, no
   merge layers.
4. Built-in bar widget ids are namespaced (`fedory.clock`, `fedory.audio`, …).
...
`config/fedory/shell.json` describes the fresh-install state. When no
...
themes don't set it, the user does via `fedory font set <name>`.
...
```json
{ "id": "vpn", "type": "command", "exec": "~/.config/fedory/bar/scripts/vpn-status",
...
Then `~/.config/fedory/bar/modules/gpu.qml` (or set `source` to point
