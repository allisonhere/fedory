...
- `default/` plays the role of `omarchy-settings`' `/etc/skel` seeding, except
  copied into an *existing* user's `$HOME` at bootstrap/finalize time instead
  of at `useradd -m` time.
