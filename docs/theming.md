...
Fedory themes live under `themes/<name>/` in the source tree (installed at
`/usr/share/fedory/themes/<name>/`), with optional user themes under
`~/.config/fedory/themes/<name>/`. A theme normally starts with a
`colors.toml`; Fedory generates the active theme files from
`default/themed/*.tpl` when `fedory-theme-set <name>` runs.

## Theme activation flow

`fedory-theme-set <name>` builds a clean staging directory at
`~/.local/state/fedory/current/next-theme`:

1. Copy the first-party theme from `themes/<name>/`.
2. Overlay any user theme files from `~/.config/fedory/themes/<name>/`.
3. If needed, generate `colors.toml` from `alacritty.toml`.
4. Run `fedory-theme-set-templates` to render templates into the staging
   theme.
5. Move the staging theme into `~/.local/state/fedory/current/theme`, write
   `~/.local/state/fedory/current/theme.name`, and notify the running shell.
...
User templates in `~/.config/fedory/themed/*.tpl` are processed before the
...
in that ramp; Fedory derives `selection_background = selection` and
`selection_foreground = bright_foreground`. Use
`fedory dev theme-preview [theme]` to inspect that ramp, including
`dark_background`, `darker_background`, and a selected-text sample.

## Template placeholders

Templates are plain files ending in `.tpl`. `fedory-theme-set-templates`
...
- Add a user-wide template under `~/.config/fedory/themed/<file>.tpl` when a
