// Notification service for the fedory shell.
...
Item {
  ...
  readonly property string stateDir: home + "/.local/state/fedory/"
  ...
  readonly property string cacheDir: home + "/.cache/fedory/"
  ...
  // Toasts are fixed to the top-right corner. They only clear the fedory bar
  ...
  PersistentProperties {
    id: persisted
    reloadableId: "fedory-notifications"
  ...
  // and rare.
  //   - fedory-action: a user-action confirmation toast ("Theme changed",
  ...
  //     Trusted because it's almost always fedory or system shell scripts —
  ...
  function handleNotification(notification) {
    ...
    //   - app_name is "fedory-action" (Fedory's own user-action
    ...
    // DND bypass rules — see ~/Work/fedory/dnd-fix-plan.md. The pending
  ...
  function focusApp(entry) {
    ...
    focusAppProc.command = [
      service.fedoryPath + "/bin/fedory-hyprland-focus-app",
      String(entry.app)
    ]
  ...
  Variants {
    ...
    PanelWindow {
      ...
      WlrLayershell.namespace: "fedory-notifications"
