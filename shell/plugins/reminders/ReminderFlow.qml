...
Item {
  ...
  function dismiss() {
    ...
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "fedory.reminders")
  }
  ...
  function submit() {
    ...
    if (root.step === "minutes") {
      ...
      if (!nextMinutes) {
        Quickshell.execDetached([root.fedoryPath + "/bin/fedory-notification-send", "Invalid reminder", "Enter the number of minutes"])
        return
      }
    ...
    if (root.step === "message") {
      var args = [root.fedoryPath + "/bin/fedory-reminder"].concat(ReminderFlowModel.reminderArgs(root.minutes, selection))
      root.dismiss()
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "fedory-reminders"
