...
Item {
  ...
  property string fedoryPath: ""

  readonly property string home: Quickshell.env("HOME")
  readonly property string stateHome: home + "/.local/state"
  readonly property string userName: Quickshell.env("USER") || Quickshell.env("LOGNAME")
  readonly property string currentBackgroundLink: stateHome + "/fedory/current/background"
  ...
  function logEvent(event) {
    ...
    console.log("fedory lock " + lastEventAt + " " + event)
  }
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "fedory-lock-preview"
  ...
  PamContext {
    id: fingerprintPam
    config: "fedory-lock-fingerprint"
    user: root.userName
  ...
  Process {
    ...
    command: ["bash", "-c", "if [[ -f /etc/pam.d/fedory-lock-fingerprint ]] && command -v fprintd-list >/dev/null 2>&1 && fprintd-list \"$USER\" 2>/dev/null | grep -qi finger; then echo yes; else echo no; fi"]
  ...
  Process {
    id: wakeProcess
    command: ["bash", "-c", "fedory-system-wake"]
  }

  Process {
    id: blankProcess
    command: ["bash", "-c", "fedory-brightness-keyboard off; fedory-brightness-display off"]
  }
  ...
  FileView {
    path: "/etc/pam.d/fedory-lock-password"
    watchChanges: true
    printErrors: false
