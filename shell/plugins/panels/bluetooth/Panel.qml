...
Panel {
  id: root
  moduleName: "fedory.bluetooth"
  ipcTarget: "fedory.bluetooth"
  // manageIpc: false so this panel can own the single IpcHandler the target
  // permits — needed for the toggleBluetooth method below.
  manageIpc: false

  // Address -> "connecting" | "disconnecting" | "forgetting".
  // The actual Bluetooth sequencing lives in bin/fedory-bluetooth-device;
  ...
  function deviceCommand(action, address) {
    return ["fedory-bluetooth-device", action, address]
  }
  ...
  // disconnects, then removes the BlueZ pairing record via fedory-bluetooth-device.
