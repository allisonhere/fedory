# Tailscale Fedory Widget

Native Fedory bar widget for Tailscale.
...
`fedory-tailscale-receive` service, which announces each one with a
notification (an image preview when the file is an image, and a click to open
it). The Tailscale service install enables it; `fedory tailscale receive`
...
This widget ships as first-party plugin `fedory.tailscale`. Add it with `fedory plugin enable fedory.tailscale`, then place it with `fedory bar move fedory.tailscale` if desired.
