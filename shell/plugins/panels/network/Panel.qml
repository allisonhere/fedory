...
Panel {
  ...
  // `fedory-network-status --verbose` samples (~1.5s apart via detailsPoll).
  ...
  // Wi-Fi band state from `fedory-network-band`. `bandCurrent` is the band
  ...
  function refresh(scanWifi) {
    ...
    if (!bandProc.running) {
      bandProc.command = ["fedory-network-band"]
  ...
  function setBand(band) {
    ...
    actionProc.command = ["fedory-network-band", band]
  ...
  function startSpeedTestPhase(phase) {
    ...
    speedTestStderr = ""
    speedTestProc.command = ["fedory-network-speedtest", phase]
  ...
  function setDns(provider) {
    ...
    if (provider === "Custom") {
      var launcher = "fedory-launch-floating-terminal-with-presentation"
  ...
  Process {
    id: detailsProc
    command: ["fedory-network-status", "--verbose"]
