...
Panel {
  id: root
  moduleName: "fedory.monitor"
  ipcTarget: "fedory.monitor"
  manageIpc: false
  ...
  // these stops; the CLI (fedory-display-text-size) accepts any integer in range.
  ...
  function setBrightness(value) {
    ...
    setBrightnessProc.command = ["fedory-brightness-display", "--no-osd", "--monitor", root.focusedMonitor, percent + "%"]
  ...
  function showBrightnessOsd(percent) {
    ...
    bar.shell.summon("fedory.osd", JSON.stringify({
      icon: "brightness",
      value: percent
    }))
  }
  ...
  function setScale(scale) {
    actionProc.command = ["bash", "-c", "fedory-hyprland-monitor-scaling " + scale]
    if (!actionProc.running) actionProc.running = true
  }
  ...
  function setTextSize(px) {
    textScaleProc.command = ["fedory-display-text-size", String(px)]
  ...
  Process {
    ...
    // `fedory-brightness-display` races the hardware/driver and can
