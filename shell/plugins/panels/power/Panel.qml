...
Panel {
  id: root
  moduleName: "fedory.power"
  ipcTarget: "fedory.power"
  ...
  function setProfile(profile) {
    ...
    actionProc.command = ["fedory-powerprofiles-set", root.discharging ? "battery" : "ac", profile]
  ...
  Process {
    id: batteryProc
    command: ["fedory-battery-status", "--shell"]
    stdout: StdioCollector { waitForEnd: true; onStreamFinished: root.updateKeyValue(text, "battery") }
  }

  Process {
    id: profilesProc
    command: ["fedory-powerprofiles-list", "--active-state"]
    stdout: StdioCollector { waitForEnd: true; onStreamFinished: root.updateProfiles(text) }
  }

  Process {
    id: systemProc
    command: ["fedory-system-stats"]
