...
Panel {
  id: root
  moduleName: "fedory.weather"
  ipcTarget: "fedory.weather"
  manageIpc: false
  ...
  // fedory-weather-location). The query is the wttr.in path segment
  ...
  property FileView locationFile: FileView {
    path: Quickshell.env("HOME") + "/.local/state/fedory/settings/weather.json"
    watchChanges: true
    printErrors: false
  ...
  function persistLocation(name, latitude, longitude) {
    if (name && latitude !== null && longitude !== null)
      locationSaveProc.command = ["fedory-weather-location", "--set", name, latitude + "," + longitude]
    else if (name)
      locationSaveProc.command = ["fedory-weather-location", "--set", name]
    else
      locationSaveProc.command = ["fedory-weather-location", "--clear"]
  ...
  // Mirrors fedory-weather-icon's wttr.in code → nerd-font glyph mapping.
