...
Panel {
  id: root
  moduleName: "fedory.audio"
  ipcTarget: "fedory.audio"

  readonly property var sink: Pipewire.defaultAudioSink
  readonly property var source: Pipewire.defaultAudioSource
  readonly property var nodes: Pipewire.nodes ? Pipewire.nodes.values : []
  readonly property var mprisPlayers: Mpris.players ? Mpris.players.values : []
  readonly property var mediaService: bar?.shell?.firstPartyServiceFor("fedory.media")
  ...
  readonly property var candidateStreams: {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      if (String(n.name || "").indexOf("fedory_speaker_tuning") === 0) continue
      list.push(n)
    }
    return list
  }
  ...
  //
  // fedory-audio-output-sink resolves the *current* default output through any
  ...
  function setDefaultSink(node) {
    ...
    if (node.id !== undefined && node.name) {
      Quickshell.execDetached([
        "fedory-audio-output-set-default",
        String(node.id),
        String(node.name)
      ])
    }
  ...
  function setDefaultSource(node) {
    ...
    if (node.id !== undefined && node.name) {
      Quickshell.execDetached([
        "fedory-audio-input-set-default",
        String(node.id),
        String(node.name)
      ])
    }
  ...
  Process {
    ...
    command: ["fedory-audio-sink-availability"]
  ...
  Process {
    id: volumeSinkProc
    command: ["fedory-audio-output-sink"]
    stdout: StdioCollector {
      waitForEnd: true
