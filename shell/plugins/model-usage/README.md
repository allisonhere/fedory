...
`fedory plugin disable fedory.model-usage`.
...
- IPC: `fedory-shell fedory.model-usage <open|close|toggle|refresh|next>`.

## Settings

Settings live in the widget's entry in `~/.config/fedory/shell.json`. The
top-level keys can be set with
`fedory bar set fedory.model-usage <key> <value>`:
...
```bash
fedory bar set fedory.model-usage refreshIntervalSec 300 --json
fedory bar set fedory.model-usage syncDir '~/Sync/model-usage'
```
...
```bash
fedory bar set fedory.model-usage providers '{
  "claude": {
    "enabled": true,
