...
Item {
  ...
  // Injected by fedory-shell (the first-party service loader).
  property var shell: null

  readonly property string home: Quickshell.env("HOME")
  readonly property string stayAwakeStateDir: home + "/.local/state/fedory/indicators"
  ...
  readonly property string screensaverClass: "org.fedory.screensaver"
  ...
  function logEvent(event, details) {
    ...
    console.log("fedory idle " + root.lastEventAt + " " + root.lastEvent)
  }
  ...
  function launchScreensaver() {
    ...
    runProcess(screensaverProcess, "screensaver", "[[ $(fedory-shell lock isLocked 2>/dev/null) == \"true\" ]] || fedory-launch-screensaver")
  }
  ...
  function lockSystem(reason) {
    ...
    runProcess(lockProcess, "lock", "fedory-system-lock")
  }
  ...
  function cancelIdleCycle(reason) {
    ...
    if (root.idledThisCycle) runProcess(wakeProcess, "wake", "fedory-system-wake")
  ...
  function persistStayAwake(value) {
    var command = value
      ? "mkdir -p \"$HOME/.local/state/fedory/indicators\" && touch \"$HOME/.local/state/fedory/indicators/stay-awake\""
      : "rm -f \"$HOME/.local/state/fedory/indicators/stay-awake\""
  ...
  Process {
    ...
    command: ["bash", "-c", "mkdir -p \"$HOME/.local/state/fedory/indicators\"; if [[ -f $HOME/.local/state/fedory/indicators/stay-awake ]]; then echo yes; else echo no; fi"]
    stdout: SplitParser {
