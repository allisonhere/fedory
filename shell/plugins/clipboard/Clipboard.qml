...
Item {
  ...
  property string historyPath: Quickshell.env("HOME") + "/.local/state/fedory/clipboard-history.json"
  property string captureScript: root.fedoryPath + "/shell/plugins/clipboard/capture.sh"
  ...
  function applySelected(row) {
    ...
    if (row.entryType === "image") {
      Quickshell.execDetached([root.fedoryPath + "/bin/fedory-clipboard-paste-file", row.mime, row.path])
    } else if (row.fullText) {
      Quickshell.execDetached([root.fedoryPath + "/bin/fedory-clipboard-paste-text", "--shift-insert", "--history-index", String(row.historyIndex)])
    }
  }
  ...
  function copySelected(row) {
    ...
    root.opened = false
    if (row.entryType === "image") {
      Quickshell.execDetached([root.fedoryPath + "/bin/fedory-clipboard-paste-file", "--copy-only", row.mime, row.path])
    } else if (row.fullText) {
      Quickshell.execDetached([root.fedoryPath + "/bin/fedory-clipboard-paste-text", "--copy-only", "--history-index", String(row.historyIndex)])
    }
  }
  ...
  function openSelected(row) {
    if (!row) return
    root.opened = false
    Quickshell.execDetached([root.fedoryPath + "/bin/fedory-clipboard-open", "--history-index", String(row.historyIndex)])
  }
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "fedory-clipboard"
