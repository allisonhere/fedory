...
Item {
  ...
  function dismiss() {
    ...
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "fedory.emojis")
  }
  ...
  function applySelected(emoji) {
    if (!emoji) return
    root.dismiss()
    Quickshell.execDetached([root.fedoryPath + "/bin/fedory-menu-emoji-insert", emoji])
  }

  ListModel { id: displayModel }

  FileView {
    path: root.fedoryPath + "/shell/plugins/emojis/emojis.json"
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "fedory-emojis"
