...
Item {
  ...
  // Injected by fedory-shell when this plugin is summoned.
  property string fedoryPath: Quickshell.env("FEDORY_PATH")
  property var shell: null
  property var manifest: null

  // Plugin lifecycle hooks. The host calls open(payloadJson) after
  // `fedory-shell shell summon fedory.menu ...` and close() when hidden.
  ...
  property string defaultMenuPath: fedoryPath + "/default/fedory/fedory-menu.jsonc"
  property string userMenuPath: Quickshell.env("HOME") + "/.config/fedory/extensions/fedory-menu.jsonc"
  ...
  readonly property var providers: ({
    "fonts": {
      script: "current=$(fedory-font-current 2>/dev/null); fedory-font-list 2>/dev/null | while read -r f; do [[ -z $f ]] && continue; printf '%s\\t%s\\t%s\\n' \"$f\" \"$f\" \"$current\"; done",
      icon: "",
      volatile: true,
      actionFor: function(value) { return "fedory-font-set " + Util.shellQuote(value) }
    },
    "power-profiles": {
      script: "current=$(powerprofilesctl get 2>/dev/null); fedory-powerprofiles-list 2>/dev/null | while read -r p; do [[ -z $p ]] && continue; printf '%s\\t%s\\t%s\\n' \"$p\" \"$p\" \"$current\"; done",
      icon: "\udb81\udc0b",
      actionFor: function(value) { return "fedory-powerprofiles-set autodetect " + Util.shellQuote(value) }
    }
  })
  ...
  // `fedory-shell shell summon fedory.menu '{"menu":"system"}'`.
  ...
  function openRoute(initialMenu) {
    ...
    // a leaf, e.g. `fedory menu summon screenrecord-stop`), run it directly
  ...
  // user extension at ~/.config/fedory/extensions/fedory-menu.jsonc) take
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "fedory-menu"
