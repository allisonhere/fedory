...
Item {
  ...
  // The fedory-shell host injects fedoryPath from FEDORY_PATH.
  required property string fedoryPath
  ...
  property string fedoryConfigDir: home + "/.config/fedory"
  ...
  // to "monospace"), so changing the system font (via `fedory-font-set`)
  ...
  // Resolve the live bar-widget instance for a plugin id (e.g. "fedory.bluetooth").
  ...
  function customModuleSource(entry) {
    var source = BarModel.customModulePath(entry, home, fedoryConfigDir)
  ...
  FileView {
    path: root.stateHome + "/fedory/current"
    watchChanges: true
    printErrors: false
  ...
  // and the flag is created/removed by `fedory-toggle-bar`.
  Process {
    id: barHiddenProbe
    running: true
    command: ["bash", "-c", "[[ -f $HOME/.local/state/fedory/toggles/bar-off ]] && echo yes || echo no"]
    stdout: SplitParser { onRead: function(line) { root.barHidden = String(line).trim() === "yes" } }
  }
  FileView {
    path: root.home + "/.local/state/fedory/toggles"
    watchChanges: true
    printErrors: false
  ...
  component BarPanel: PanelWindow {
    ...
    WlrLayershell.namespace: "fedory-bar"
  ...
  component DragGhostPanel: PanelWindow {
    ...
    WlrLayershell.namespace: "fedory-bar-drag-ghost"
  ...
  component BarMoveGhostPanel: PanelWindow {
    ...
    color: "transparent"
    exclusionMode: ExclusionMode.Ignore
    WlrLayershell.namespace: "fedory-bar-move-ghost"
    WlrLayershell.layer: WlrLayer.Overlay
    WlrLayershell.keyboardFocus: WlrKeyboardFocus.None
