...
ShellRoot {
  ...
  // The fedory-shell host is the long-running entry point. Plugins live in
  // sibling directories under plugins/. FEDORY_PATH is provided by the uwsm
  // session environment and is the single source of truth for this checkout.
  property string fedoryPath: Quickshell.env("FEDORY_PATH")
  readonly property string shellPath: fedoryPath + "/shell"
  readonly property string firstPartyPluginsDir: shellPath + "/plugins"
  readonly property string defaultsPath: fedoryPath + "/config/fedory/shell.json"
  readonly property string userConfigPath: home + "/.config/fedory/shell.json"
  ...
  readonly property var builtinShellConfig: ({
    ...
    bar: {
      ...
      layout: {
        ...
        center: [{ id: "fedory.clock", format: "dddd HH:mm" }],
  ...
  Component.onCompleted: {
    console.log("fedory-shell paths",
      "fedoryPath=" + shell.fedoryPath,
  ...
  readonly property string defaultBarId: "fedory.bar"
  ...
  function configureBar(target, manifest) {
    if (!target) return
    if ("fedoryPath" in target) target.fedoryPath = shell.fedoryPath
  ...
  function ensureService(pluginId) {
    ...
    function finalize() {
      ...
      if ("fedoryPath" in inst) inst.fedoryPath = shell.fedoryPath
  ...
  function isBarWidgetPanelPlugin(pluginId) {
    ...
    // panel loader (e.g. fedory.menu); let that path handle them.
  ...
  Instantiator {
    ...
    delegate: QtObject {
      ...
      property Loader panelLoader: Loader {
        ...
        onLoaded: {
          if (!item) return
          if ("fedoryPath" in item) item.fedoryPath = shell.fedoryPath
  ...
  function imagePickerItem() {
    var loader = panelLoaders["fedory.image-picker"]
  ...
  IpcHandler {
    ...
    function open(imageDirs: string,
      ...
      return shell.summon("fedory.image-picker", payload) ? "ok" : "unknown"
    }
    ...
    function cancel(doneFile: string): string {
      ...
      if (picker && typeof picker.closeSelector === "function") {
        ...
        shell.hide("fedory.image-picker")
      }
      return "ok"
    }
  ...
  IpcHandler {
    ...
    function listPlugins(): string {
      ...
      for (var id in plugins) {
        ...
        var metadata = plugins[id].fedory
        var clonedFrom = Util.isPlainObject(metadata) ? String(metadata.clonedFrom || "") : ""
        out.push({
          id: id,
          name: plugins[id].name,
          kinds: kinds,
          // What `fedory plugin enable/disable` toggles: for a widget that is
