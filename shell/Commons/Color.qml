...
QtObject {
  ...
  readonly property string currentThemePath: stateHome + "/fedory/current/theme"
  ...
  // Style. User keys win, so a machine-level `~/.config/fedory/shell.toml`
  ...
  // is where `fedory display text size` writes `[font] base-size`. Watched so the
  // CLI takes effect live without restarting the shell; absent by default.
  property FileView userShellFile: FileView {
    id: userShellFile
    path: root.home + "/.config/fedory/shell.toml"
    watchChanges: true
    printErrors: false
