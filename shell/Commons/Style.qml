...
QtObject {
  ...
  // component follows the fontconfig alias `fedory-font-set` writes.
  ...
  // alias path keeps working when the user runs `fedory font set`), but
  ...
  // FEDORY_MENU_FONT override for users who want a different family on the
  ...
  readonly property string menuFontFamily: {
    var override = Quickshell.env("FEDORY_MENU_FONT")
  ...
  // Resolve the fontconfig alias to a concrete family name. `fedory font
  ...
  // `fedory toggle window-gaps` creates/removes this flag file. Hyprland
  ...
  property FileView windowNoGapsToggle: FileView {
    path: Quickshell.env("HOME") + "/.local/state/fedory/toggles/hypr/window-no-gaps.lua"
    watchChanges: true
    printErrors: false
