...
QtObject {
  ...
  property string pluginsDir: home + "/.config/fedory/plugins"
  ...
  //   - the built-in bar option (`fedory.bar`) is active when `bar.id` is
  //     missing or set to `fedory.bar`.
  ...
  //     silently make `fedory launch bar-settings` a no-op. Turning one off
  ...
  function isEnabled(id) {
    ...
    if (manifest) {
      if (Array.isArray(manifest.kinds) && manifest.kinds.indexOf("bar") !== -1) {
        ...
        if (!selectedBar) selectedBar = "fedory.bar"
  ...
  function resolveEnabledId(id) {
    ...
    for (var candidate in installedPlugins) {
      ...
      var metadata = manifest && Util.isPlainObject(manifest.fedory) ? manifest.fedory : null
  ...
  // back, and so a plugin that is both a widget and a menu (fedory.menu)
  ...
  function barTarget(config, placement, fallbackSection) {
    ...
    var anchors = { left: "fedory.workspaces", center: "fedory.weather", right: "fedory.tray" }
  ...
  function activeCloneFor(config, sourceId) {
    for (var candidate in installedPlugins) {
      ...
      var candidateMetadata = candidateManifest && Util.isPlainObject(candidateManifest.fedory)
        ? candidateManifest.fedory : null
  ...
  function restoreCloneSource(config, cloneId, sourceId) {
    ...
    if (isBarOption) {
      if (sourceId === "fedory.bar") delete config.bar.id
    ...
    shellConfigMutator(function(config) {
      ...
      if (isBarOption) {
        if (value) {
          ...
          if (clonedFrom && clonedFrom !== "fedory.bar") config.bar.id = clonedFrom
  ...
  function parseScanOutput(text) {
    ...
    // `fedory.*` namespace is reserved for built-ins, including bar widgets
    // registered outside the manifest-based plugin registry.
    for (var tk in thirdParty) {
      if (firstParty[tk] || String(tk).indexOf("fedory.") === 0) {
        console.warn("PluginRegistry: plugin " + tk
          + " rejected: id is reserved for first-party Fedory plugins")
        continue
      }
  ...
  function rescan() {
    ...
    // Third-party plugins stay at the top level of ~/.config/fedory/plugins.
    var script = ""
