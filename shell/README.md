...
`fedory-shell` is a single long-running [Quickshell](https://quickshell.org/)
instance that hosts the Fedory desktop. Hyprland autostart launches one shell
...
| `bar`        | A full bar option that can replace the built-in `fedory.bar` |

Only one `bar` plugin is active at a time. Missing or invalid selections fall
back to the built-in `fedory.bar`, so users always have a safe path home.
...
clones it straight into `~/.config/fedory/plugins/<id>/` (named by the
manifest id); updating is a fast-forward pull of that checkout.

```bash
fedory plugin add https://github.com/acme/fedory-weather.git
fedory plugin update acme.weather       # fetches, shows a diff, fast-forwards
fedory plugin update                    # updates every git-managed plugin
fedory plugin remove acme.weather
```

> ⚠️ **Plugins run as unsandboxed code inside `fedory-shell`.** Adding warns
...
```bash
fedory plugin add https://github.com/acme/fedory-weather.git --enable --yes
fedory plugin update --yes
```
...
1. Put it in `~/.config/fedory/plugins/<plugin-id>/` with a `manifest.json`
   plus the QML referenced from its `entryPoints`.
2. `fedory-shell shell rescanPlugins`.
3. `fedory plugin enable <id>`. Bar widgets start in
   `barWidget.defaultSection`, or in the center when it is omitted, and can be
   moved with `fedory bar move`; a full bar replaces the one in use.

The lower-level IPC equivalents remain available via `fedory-shell shell rescanPlugins`,
`fedory-shell shell enablePlugin <id> '{}'`, and `fedory-shell shell listPlugins`.
The `fedory plugin` commands wrap those calls. `fedory bar move` and
`fedory bar set` edit the persisted widget layout in `shell.json`.
...
`fedory.clock` becomes `<username>.clock` (for example, `allie.clock`), with
`My Clock` as its display name. The username prefix prevents shared clones
from colliding with each other or other plugin authors.

```bash
fedory plugin clone fedory.clock
```
...
Saving a file anywhere under `~/.config/fedory/plugins/` reloads plugin code
automatically; `fedory-shell shell rescanPlugins` remains available to force a reload.
...
hooks, the image picker's `image-selector` target). `fedory-menu` uses the
shell target to summon the first-party `fedory.menu` plugin instead of
...
| `reloadConfig`                           | `ok`    | reload `~/.config/fedory/shell.json`                 |
...
```
quickshell ipc -p $FEDORY_PATH/shell call shell ping
```

Hyprland autostart launches the shell directly with `quickshell -p
$FEDORY_PATH/shell`. Use `fedory-restart-shell` to stop every running
instance of that config and launch one fresh shell process.

A convenience wrapper, [`fedory-shell`](../bin/fedory-shell), forwards IPC
calls to the running shell. It does not start the shell.

```
fedory-shell shell ping
fedory-shell shell toggle fedory.menu '{"menu":"root"}'
fedory-shell shell listPlugins
fedory-shell shell rescanPlugins
```
...
| Path                              | Owner          | Purpose                                                |
|-----------------------------------|----------------|--------------------------------------------------------|
| `~/.config/fedory/shell.json`    | the shell      | full layout + per-entry settings + enabled plugin list |
| `~/.config/fedory/plugins/<id>/` | user           | drop-in third-party plugin source files                |

The `config/fedory/shell.json` default config describes the
...
```json
{
  ...
  "bar": {
    ...
    "layout": {
      "left":   [ { "id": "fedory.menu" }, { "id": "fedory.workspaces" } ],
...
1. **The active bar option is `bar.id`.** Omit it or set it to `fedory.bar`
...
4. **Built-in widget ids are namespaced.** Use ids such as `fedory.clock`,
   `fedory.audio`, and `fedory.network`. The migration rewrites older ids
...
6. **Multiple instances** are allowed when a manifest sets
   ...
   widgets in different timezones are just two `{"id":"fedory.clock", "timezone": ...}`
...
- Phase 1 — `fedory-shell phase 1: host the existing bar in a single shell`
- Phase 2 — `fedory-shell phase 2: plugin registry and bar widget registry`
- Phase 3 — `fedory-shell phase 3: fold bar-settings into the shell as a panel plugin`
- Phase 4 — `fedory-shell phase 4: absorb background-switcher as a plugin`
- Phase 5 — `fedory-shell phase 5: docs, cleanup, and migration crumbs`
- Phase 6 — `fedory-shell phase 6: reviewer cleanup (path traversal, collision, races)`
- Phase 7 — `fedory-shell phase 7: replace socket with IpcHandler, rename to image-picker`
- Phase 8a — `fedory-shell phase 8a: unified shell.json with inline plugin settings`
