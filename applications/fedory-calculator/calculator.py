#!/usr/bin/env python3
"""Fedory Calculator — Tray icon calculator for Fedory.

Based on remigius-labs/omacalc at 922af36, released under the MIT License.
Fedory changes the runtime paths, application identity, library discovery, and
launcher integration while preserving the calculator behavior.
"""
...
def load_theme_colors():
    path = os.path.expanduser("~/.local/state/fedory/current/theme/colors.toml")
...
SNI_BUS_NAME = "org.kde.StatusNotifierItem-fedory-calculator"
...
def make_icon_svg(accent_hex, fg_hex):
    """Create a cute calculator SVG tray icon themed to match Fedory.
...
def write_icon_files(accent_hex, fg_hex):
    ...
    icon_dir = os.path.join(runtime, "fedory-calculator")
    os.makedirs(icon_dir, exist_ok=True)

    svg_content = make_icon_svg(accent_hex, fg_hex)

    svg_path = os.path.join(icon_dir, "fedory-calculator.svg")
    with open(svg_path, "w") as f:
        f.write(svg_content)

    hicolor = os.path.expanduser("~/.local/share/icons/hicolor/scalable/apps")
    os.makedirs(hicolor, exist_ok=True)
    with open(os.path.join(hicolor, "fedory-calculator.svg"), "w") as f:
        f.write(svg_content)
...
class StatusNotifierItem(dbus.service.Object):
    ...
    def _get_prop(self, prop):
        ...
        if prop == "Id":
            return dbus.String("fedory-calculator")
        if prop == "Title":
            return dbus.String("Fedory Calculator")
        if prop == "Status":
...
class CalcWindow(Gtk.Window):
    def __init__(self, app):
        super().__init__(application=app, title="Fedory Calculator")
        ...
        Gtk4LayerShell.set_namespace(self, "fedory-calculator")
...
class FedoryCalculatorApp(Gtk.Application):
    def __init__(self, session_bus, bus_name_obj, sni):
        super().__init__(
            application_id="com.fedory.calculator",
    ...
    def _init_window(self):
        ...
        current_dir = os.path.expanduser("~/.local/state/fedory/current")
...
def ensure_layer_shell_preload():
    ...
    lib = next((path for path in candidates if os.path.exists(path)), "")
    if not lib:
        return
...
Name=Fedory Calculator
Comment=Tray calculator for Fedory
Exec=fedory-calculator
StartupNotify=false
Terminal=false
Type=Application
"""
    dest = os.path.join(autostart_dir, "fedory-calculator.desktop")
...
def main():
    ...
    app = FedoryCalculatorApp(session_bus, bus_name_obj, sni)
    app.run(None)
